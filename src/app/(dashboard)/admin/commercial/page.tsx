        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center gap-2">
                  <h3 className="font-semibold text-gray-900">1. Planes de suscripción</h3>
                  <span className="text-xs text-gray-500">Crea y administra los planes disponibles en la plataforma.</span>
                </div>
                <div className="flex items-center gap-2">
              ...
              <CardContent className="p-6 pt-0">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {plans?.map((plan: Plan, index: number) => (
                      <div
                        key={plan.id}
                        className={cn(
                          'p-5 rounded-xl border-2 relative',
                      ...
                      >
                        ...
                        <p className="text-2xl font-bold text-gray-900 mb-4">
                          C$ {plan.price.toLocaleString()}<span className="text-sm font-normal text-gray-500">/mes</span>
                        </p>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Button variant="outline" size="sm" className="flex-1 text-red-600 hover:bg-red-50" onClick={() => { if (confirm('¿Eliminar este plan?')) deletePlanMutation.mutate(plan.id); }}>
                ...
                <p className="text-xs text-gray-500 mt-4 flex items-center gap-2">
                  <span className="h-4 w-4 rounded-full bg-gray-200 flex items-center justify-center text-gray-500">i</span>
                  Los planes pueden asignarse a las empresas desde su perfil.
                  <a href="/admin/companies" className="text-blue-600 hover:underline">Gestionar empresas →</a>
                </p>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center gap-2">
                  <h3 className="font-semibold text-gray-900">2. Pagos y facturación reciente</h3>
                  <span className="text-xs text-gray-500">Revisa los pagos realizados y registra cobros manuales.</span>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {paymentsData?.payments?.map((payment: Payment) => (
                      <tr key={payment.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <div className="h-8 w-8 rounded bg-blue-100 flex items-center justify-center">
                              <span className="text-sm font-medium text-blue-600">{payment.company.name.charAt(0)}</span>
                            </div>
                            <div>
                              <p className="font-medium text-gray-900 text-sm">{payment.company.name}</p>
                              <p className="text-xs text-gray-500">{payment.company.city}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          <span className={cn('px-2 py-1 rounded-full text-xs font-medium', statusColors[payment.status] || 'bg-gray-100 text-gray-700')}>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-1">
                            <button className="p-1.5 hover:bg-gray-100 rounded" title="Ver detalle">
                              <Eye className="h-4 w-4 text-gray-500" />
                            </button>
                            <button className="p-1.5 hover:bg-gray-100 rounded" title="Descargar recibo">
                              <Download className="h-4 w-4 text-gray-500" />
                            </button>
                            {payment.status === 'pending' && (
                              <button className="p-1.5 hover:bg-gray-100 rounded" title="Enviar recordatorio">
                      ...
                      <tr>
                        <td colSpan={6} className="px-6 py-8 text-center text-gray-500">No hay pagos registrados</td>
                      </tr>
                    )}
                  </tbody>
                </table>
                      ...
                      {[...Array(Math.min(5, paymentsData.totalPages))].map((_, i) => (
                        <button
                          ...
                          className={cn(
                            ...
                            paymentFilters.page === i + 1 ? 'bg-blue-600 text-white' : 'hover:bg-gray-100'
                          )}
                        >
                          {i + 1}
                        </button>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-semibold text-gray-900">Asigna un plan a una empresa</p>
                      <p className="text-sm text-gray-500">Cambia o asigna el plan activo de cualquier empresa.</p>
                    </div>
                  </div>
                  <Button variant="outline" onClick={() => setShowAssignPlanModal(true)}>
            ...
            <Card className="sticky top-6">
              <CardHeader>
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-semibold text-gray-900">3. Configuración comercial</h3>
                </div>
                <span className="text-xs text-gray-500">Parámetros y reglas que aplican a toda la plataforma.</span>
              </CardHeader>
              <CardContent className="p-6 pt-0 space-y-6">
                {/* Publication Limits */}
                <div>
                  <h4 className="font-medium text-gray-900 mb-3">Límites de publicaciones</h4>
                  <div className="space-y-3">
                    <div>
                      <label className="text-sm text-gray-600">Límite publicaciones - Básico</label>
                      <div className="flex items-center gap-2">
                        <input
                          type="number"
                          value={localSettings?.limitBasico ?? 10}
                          onChange={e => setLocalSettings({ ...localSettings!, limitBasico: +e.target.value })}
                          className="flex-1 h-9 rounded-lg border border-gray-300 px-3 text-sm"
                        />
                        <span className="text-sm text-gray-500">por mes</span>
                      </div>
                    </div>
                    <div>
                      <label className="text-sm text-gray-600">Límite publicaciones - Profesional</label>
                      <div className="flex items-center gap-2">
                        <input
                          type="number"
                    ...
                    <div>
                      <label className="text-sm text-gray-600">Límite publicaciones - Empresarial</label>
                      <div className="flex items-center gap-2">
                        <input
                          type="number"
                        ...
                        />
                        <span className="text-sm text-gray-500">ilimitadas</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <h4 className="font-medium text-gray-900 mb-3">Visibilidad de candidatos</h4>
                  <div className="space-y-3">
                    <label className="flex items-center justify-between">
                      <span className="text-sm text-gray-600">Mostrar candidatos a empresas</span>
                      <input
                        type="checkbox"
                        ...
                        className="h-5 w-9 rounded-full appearance-none bg-gray-200 checked:bg-blue-600 relative cursor-pointer transition-colors before:content-[''] before:absolute before:h-4 before:w-4 before:bg-white before:rounded-full before:top-0.5 before:left-0.5 before:transition-transform checked:before:translate-x-4"
                      />
                    </label>
                    <p className="text-xs text-gray-500">Permite a las empresas ver perfiles de candidatos.</p>

                    <label className="flex items-center justify-between">
                      <span className="text-sm text-gray-600">Mostrar salario esperado</span>
                      <input
                        type="checkbox"
                    ...
                    <p className="text-xs text-gray-500">Muestra el salario esperado en los perfiles.</p>

                    <label className="flex items-center justify-between">
                      <span className="text-sm text-gray-600">Datos de contacto visibles</span>
                      <input
                        type="checkbox"
                    ...
                    <p className="text-xs text-gray-500">Permite mostrar datos de contacto a empresas.</p>
                  </div>
                </div>

                {/* Commercial Rules */}
                <div>
                  <h4 className="font-medium text-gray-900 mb-3">Reglas comerciales</h4>
                  <div className="space-y-3">
                    <div>
                      <label className="text-sm text-gray-600">Período de facturación</label>
                      <select
                        value={localSettings?.billingPeriod ?? 'monthly'}
                        onChange={e => setLocalSettings({ ...localSettings!, billingPeriod: e.target.value })}
                        className="w-full h-9 rounded-lg border border-gray-300 px-3 text-sm mt-1"
                      >
                    ...
                    <div>
                      <label className="text-sm text-gray-600">Días de gracia por impago</label>
                    ...
                    <div>
                      <label className="text-sm text-gray-600">Recordatorios de pago</label>
                      <select
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-md rounded-xl bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
          <h2 className="text-lg font-semibold text-gray-900">{plan ? 'Editar plan' : 'Nuevo plan'}</h2>
        ...
        <div className="p-6">
          <form onSubmit={(e) => { e.preventDefault(); onSubmit(form); }} className="space-y-4">
            {!plan && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">ID del plan *</label>
                <input type="text" value={form.id} onChange={e => setForm({ ...form, id: e.target.value })} className="w-full h-10 rounded-lg border border-gray-300 px-3" required />
              </div>
            )}
            <div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Precio mensual (C$) *</label>
              <input type="number" value={form.price} onChange={e => setForm({ ...form, price: +e.target.value })} className="w-full h-10 rounded-lg border border-gray-300 px-3" required />
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Publicaciones</label>
                <input type="number" value={form.publicationLimit} onChange={e => setForm({ ...form, publicationLimit: +e.target.value })} className="w-full h-10 rounded-lg border border-gray-300 px-3" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Usuarios</label>
                <input type="number" value={form.userLimit} onChange={e => setForm({ ...form, userLimit: +e.target.value })} className="w-full h-10 rounded-lg border border-gray-300 px-3" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Candidatos</label>
                <input type="number" value={form.visibleCandidatesLimit} onChange={e => setForm({ ...form, visibleCandidatesLimit: +e.target.value })} className="w-full h-10 rounded-lg border border-gray-300 px-3" />
              </div>
            </div>
            <p className="text-xs text-gray-500">Use -1 o 0 para ilimitado.</p>
...
function PaymentModal({ companies, plans, onClose, onSubmit, isLoading }: { companies?: { id: string; name: string }[]; plans?: Plan[]; onClose: () => void; onSubmit: (data: Record<string, unknown>) => void; isLoading: boolean }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-md rounded-xl bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
          <h2 className="text-lg font-semibold text-gray-900">Registrar cobro manual</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-2xl">&times;</button>
        </div>
        <div className="p-6">
          <form onSubmit={(e) => { e.preventDefault(); onSubmit(form); }} className="space-y-4">
            <div>
              ...
              <select value={form.companyId} onChange={e => setForm({ ...form, companyId: e.target.value })} className="w-full h-10 rounded-lg border border-gray-300 px-3" required>
            ...
            <div>
              ...
              <select value={form.planId} onChange={e => setForm({ ...form, planId: e.target.value })} className="w-full h-10 rounded-lg border border-gray-300 px-3" required>
                <option value="">Seleccionar plan...</option>
                {plans?.map(p => <option key={p.id} value={p.id}>{p.name} - C$ {p.price.toLocaleString()}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Monto (C$) *</label>
              <input type="number" value={form.amount} onChange={e => setForm({ ...form, amount: +e.target.value })} className="w-full h-10 rounded-lg border border-gray-300 px-3" required />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Referencia</label>
              <input type="text" value={form.reference} onChange={e => setForm({ ...form, reference: e.target.value })} className="w-full h-10 rounded-lg border border-gray-300 px-3" placeholder="No. de transacción" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Notas</label>
              <textarea value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} className="w-full rounded-lg border border-gray-300 px-3 py-2" rows={2} />
            </div>
            <div className="flex justify-end gap-3 pt-4">
              <Button variant="outline" type="button" onClick={onClose}>Cancelar</Button>
...
function AssignPlanModal({ companies, plans, onClose, onSubmit, isLoading }: { companies?: { id: string; name: string; plan?: { id: string; name: string } }[]; plans?: Plan[]; onClose: () => void; onSubmit: (data: { companyId: string; planId: string }) => void; isLoading: boolean }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-md rounded-xl bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
          <h2 className="text-lg font-semibold text-gray-900">Asignar plan a empresa</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-2xl">&times;</button>
        </div>
        <div className="p-6">
          <form onSubmit={(e) => { e.preventDefault(); onSubmit(form); }} className="space-y-4">
