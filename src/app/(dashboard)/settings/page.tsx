...
export default function SettingsPage() {
  ...
  const [companyDraft, setCompanyDraft] =
    useState<Partial<CompanyDraft> | null>(null);
  ...
  const form = useMemo(() => {
    return {
      ...
      secondaryPhone:
        companyDraft?.secondaryPhone ?? company?.secondaryPhone ?? "",
  ...
  const createUserMutation = useMutation({
    mutationFn: (payload: Record<string, unknown>) =>
      companies.createMyUser(payload),
    onSuccess: () => {
  ...
  const deleteUserMutation = useMutation({
    mutationFn: async (id: string) => {
      try {
        ...
        return await companies.updateMyUser(id, {
          status: "inactive",
          userId: id,
        });
      }
  ...
  const usage =
    planLimits?.usage ?? billingPlan?.usage ?? billingPlan?.limits ?? {};
  const currentPlanFromList = (availablePlans ?? []).find(
    (p: Plan) => p.id === currentPlanId,
  );
    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      <Header
        ...
        actions={
          <Button
            className="h-11"
            onClick={() => updateCompanyMutation.mutate(form)}
            ...
            <CardContent className="space-y-5 p-5">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 text-sm font-bold text-[#0B5CFF] hover:text-[#004BDD]"
                >
                ...
                <div className="md:col-span-4">
                  <div className="relative flex h-40 flex-col items-center justify-center rounded-xl border border-[#E6ECF5] bg-[#F8FAFC] overflow-hidden">
                    {currentLogo ? (
                      ...
                      <Building2 className="h-12 w-12 text-[#0B5CFF]" />
                    )}
                  </div>
                  <input
                    ref={logoInputRef}
                ...
                <div className="space-y-3 md:col-span-8">
                  <Field label="Nombre de la empresa">
                    <input
                      ...
                      className="h-11 w-full rounded-xl border border-[#E6ECF5] px-3 text-sm disabled:bg-[#F8FAFC]"
                    />
                  </Field>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-sm font-bold text-[#0F172A]">
                                ...
                                {index === 0 ? (
                                  <span className="rounded-md bg-[#EAF2FF] px-1.5 py-0.5 text-[11px] text-[#0B5CFF]">
                                    Tu
                                  </span>
                                ) : null}
                              </p>
            ...
            <CardContent className="space-y-4 p-5">
              ...
              <div className="grid grid-cols-2 gap-3 rounded-xl border border-[#E6ECF5] p-4">
                <Metric
                  label="Publicaciones"
                  current={
                    usage?.jobs?.current ?? usage?.activeJobs?.current ?? 0
                  }
                  max={usage?.jobs?.max ?? usage?.activeJobs?.max ?? 0}
                />
                <Metric
                  label="Usuarios"
                ...
                />
                <Metric
                  label="Visibilidad candidatos"
                  current={
                    usage?.candidates?.current ??
                    usage?.visibleCandidates?.current ??
                    0
                  }
                  max={
                    usage?.candidates?.max ?? usage?.visibleCandidates?.max ?? 0
                  }
                />
                <Metric
                  label="Vacantes activas"
                  current={usage?.activeJobs?.current ?? 0}
                  max={usage?.activeJobs?.max ?? 0}
                />
              </div>
              ...
              <div className="rounded-xl border border-[#DCEBFF] bg-[#F8FBFF] px-4 py-3">
                ...
                <p className="text-xs text-[#64748B]">
                  Actualiza tu plan para aumentar los límites y seguir
                  creciendo.
                </p>
                <Button
                  className="mt-2 h-9"
              ...
              <CardContent className="space-y-4 p-5">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-[#64748B]">
                      Elige el plan que mejor se adapte al crecimiento de tu
                      empresa.
                    </p>
                  </div>
                  <div className="flex items-center rounded-xl border border-[#E6ECF5] p-1">
                    <button
                      ...
                      className={cn(
                        ...
                        !annual ? "bg-[#0B5CFF] text-white" : "text-[#334155]",
                      )}
                    >
                      Mensual
                    </button>
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="text-4xl font-bold text-[#0F172A]">
                          C$ {Math.round(price).toLocaleString()}{" "}
                        ...
                        <ul className="mt-4 space-y-2 text-sm text-[#475569]">
                          <li className="inline-flex items-center gap-2">
                            ...
                            {(plan.activeJobs ??
                              (plan as Plan & { publicationLimit?: number })
                                .publicationLimit ??
                              0) > 0
                              ? `${plan.activeJobs ?? (plan as Plan & { publicationLimit?: number }).publicationLimit} publicaciones por mes`
                              : "Publicaciones ilimitadas"}
                          </li>
                          <li className="inline-flex items-center gap-2">
                            <Check className="h-4 w-4 text-[#16A34A]" />{" "}
                            {(plan.users ??
                              (plan as Plan & { userLimit?: number })
                                .userLimit ??
                              0) > 0
                              ? `${plan.users ?? (plan as Plan & { userLimit?: number }).userLimit} usuarios`
                              : "Usuarios ilimitados"}
                          </li>
                          <li className="inline-flex items-center gap-2">
                            <Check className="h-4 w-4 text-[#16A34A]" />{" "}
                            {(plan.visibleCandidates ??
                              ...
                              0) > 0
                              ? `${plan.visibleCandidates ?? (plan as Plan & { visibleCandidatesLimit?: number }).visibleCandidatesLimit} candidatos visibles`
      ...
      {planReference ? (
        <div className="fixed bottom-4 right-4 rounded-xl bg-[#0F172A] px-4 py-2 text-xs font-semibold text-white">
          Referencia guardada: {planReference}
        </div>
      ) : null}
  ...
  return (
    <div>
      ...
      <div className="mt-1 h-2 rounded-full bg-[#E6ECF5]">
        <div
          className="h-2 rounded-full bg-[#0B5CFF]"
          style={{ width: `${pct}%` }}
        />
      </div>
      <p className="mt-1 text-xs text-[#64748B]">
        restantes {Math.max(0, max - current)}
      </p>
    </div>
  );
}
...
function UserModal({
  ...
  const [role, setRole] = useState(
    user?.companyRole ?? user?.role ?? "recruiter",
  );
      ...
      <div className="w-full max-w-lg rounded-2xl bg-white">
        ...
        <div className="space-y-3 p-5 text-sm text-[#334155]">
          ...
          <div className="rounded-xl border border-[#E6ECF5] bg-[#F8FAFC] p-3">
            <p>
              <strong>BAC:</strong> 123-456-789
            </p>
            <p>
              <strong>LAFISE:</strong> 987-654-321
            </p>
            <p>
              <strong>BANPRO:</strong> 456-789-123
            </p>
          </div>
          <input
            value={reference}
